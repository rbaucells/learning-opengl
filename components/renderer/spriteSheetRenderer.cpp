...
std::unique_ptr<Tween<float>> SpriteSheetRenderer::alphaTween(float start, float end, float duration, const Curve& curve) {
    return std::make_unique<Tween<float>>(&alpha, start, end, duration, curve);
}

std::unique_ptr<Tween<float>> SpriteSheetRenderer::alphaTween(float end, float duration, const Curve& curve) {
