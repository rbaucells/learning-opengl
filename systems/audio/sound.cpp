...
Sound::Sound(const std::string& filePath) {
    ...
    in.read(data_, header_.size);
}

std::uint8_t Sound::getHeaderChannels() const {
    return header_.channels;
}

std::int32_t Sound::getHeaderSampleRate() const {
    return header_.sampleRate;
}

std::uint8_t Sound::getHeaderBitsPerSample() const {
    return header_.bitsPerSample;
}

ALsizei Sound::getHeaderSize() const {
    return header_.size;
}
...
Sound::WavHeader Sound::loadWavFileHeader(std::ifstream& file) {
    ...
    const std::uint8_t channels = reinterpret4BytesAsInt(buffer, 2);
    ...
    const std::int32_t sampleRate = reinterpret4BytesAsInt(buffer, 4);
    ...
    const std::uint8_t bitsPerSample = reinterpret4BytesAsInt(buffer, 2);
    ...
    const ALsizei size = reinterpret4BytesAsInt(buffer, 4);
